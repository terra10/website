...
import {WordpressFeedType} from './interfaces/wordpressFeedType'; 
...
export async function getWordpressFeed(event: APIGatewayProxyEvent) { 
    ...
    try { 
        ...
        const response = await fetch('https://www.rubix.nl/category/terra10-blogs/feed/', { 
            method: 'GET', 
            headers: { 
                'Content-Type': 'application/json' 
            } 
        ...
        const responseJson: WordpressFeedType = await convert.xmlDataToJSON(responseXml); 
